...
agenda_melhor1['lucas']
agenda_melhor1['lucas']['email']
...
def conta_telefones(agenda):
    qtdTelefones = 0
    for pessoa in agenda.keys():
        qtdTelefones += len(agenda[pessoa]['telefones'])
    return qtdTelefones
    ...
    for pessoa in agenda.keys():
        if 'telefones' in agenda[pessoa]:
            for telefone in agenda[pessoa]['telefones']:
                if telefone not in contagens.keys():
                    contagens[telefone] = 0
                contagens[telefone] += 1
    return contagens
